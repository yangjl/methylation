...
gen <- read.csv("cache/SFS_comet_features_CHG.csv")
...
###################################################################################################
### SFS and transposons
tab4 <- read.csv("cache/SFS_comet_features_CHG.csv")
...
pdf("graphs/Figure_sfs_chg.pdf", width=6, height=12)
ggdraw(p) + draw_plot_label(c("E", "F", "G", "H" ), c(0, 0, 0, 0), c(1, 3/4, 2/4, 1/4), size=15)
dev.off()
